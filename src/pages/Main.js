      ...
      <View style={styles.cardsContainer}>
        {
          users.length === 0
            ...
            : (
                users.map((user, key) => (
                  <View key={key} style={[styles.card, { zIndex: users.length - key }]}>
                    <Image  style={styles.avatar} source={{ uri: user.avatar }} />
                    <View style={styles.description}>
                      <Text style={styles.name}>{user.name}</Text>
                      <Text numberOfLines={3} style={styles.bio}>{user.bio}</Text>
                      <Text style={styles.bio}>{user.user}</Text>
                    </View>
                  </View>
                ))
              )
...

      {
        users.length > 0 && (
          <View style={styles.buttonsContainer}>
            <TouchableOpacity onPress={handleDislike} style={styles.button}><Image source={dislike} /></TouchableOpacity>
